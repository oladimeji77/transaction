...
def create_user(request: schema.CreateUser, db: Session = Depends(get_db)):
    ...
    new_user = model.UserDB(**request.dict())
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return new_user
...
def specific_users(user_id: int, db: Session = Depends(get_db), current_user: schema.TranxGLDb = Depends(get_current_user)):
    single_user = db.query(model.UserDB).filter(model.UserDB.id == user_id).first()
    if single_user == None:
...
def delete_user(user_id: int, db: Session = Depends(get_db), current_user: int = Depends(get_current_user)):
    #todelete = db.query(model.UserDB).filter(model.UserDB.id == user_id).delete(synchronize_session=False)
    todelete = db.query(model.UserDB).filter(model.UserDB.id == user_id)
